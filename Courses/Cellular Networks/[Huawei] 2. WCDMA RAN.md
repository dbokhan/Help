...
Радиоинтерфейс выполняется как на контроллере, так и на базовой станции. В UMTS RRC выполняет контроллер, на контроллере функции RRC выполняет сингнальный процессор. Функции обработки второго уровня (PDCP, RLC и частично MAC) на контроллере выполняют платы Data Proccessing'а.
...
Все сервисы попадают на уровень **RLC (Radio Link Control)**. 